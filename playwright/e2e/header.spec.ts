...
test.describe("Header", () => {
  ...
  test("renders correctly", async () => {
    ...
    await expect(navItems.nth(0)).toHaveText("CASEI")
    await expect(navItems.nth(1)).toHaveText("Explore")
    await expect(navItems.nth(2)).toContainText("Learn")
    await expect(navItems.nth(3)).toHaveText("Glossary")
    await expect(navItems.nth(4)).toHaveText("FAQ")
