...
test.describe("Platform", () => {
    ...
    test("explains the platform", async ({ page }) => {
        ...
        expect(await statsItems[1].textContent()).toContain("Collection Periods");
        ...
        await page.waitForSelector("[data-cy=campaigns-instruments-section] h2", { visible: true });
        expect(await page.$eval("[data-cy=campaigns-instruments-section] h2", el => el.textContent)).toContain("Related Campaigns & Instruments");
        ...
        await page.waitForSelector("[data-cy=campaign-carousel] .slider-control-centerright > button");

        await page.waitForSelector("[data-cy=instrument-accordion]", { visible: true });
        await page.click("[data-cy=instrument-accordion] [data-cy=accordion-button]:first-child");
        await page.waitForSelector("[data-cy=instrument-accordion-content] [data-cy=instrument-accordion-image-description]");
