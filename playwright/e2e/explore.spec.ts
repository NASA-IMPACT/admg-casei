...
test.describe('Explore', () => {
    ...
    test.describe('campaigns', () => {
        ...
        test('displays campaign cards and navigates to the selected campaign', async ({ page }) => {
            ...
            await expect(page.locator('[data-cy=campaigns-count]').first()).toBeVisible();
            await expect(page.locator('[data-cy=campaigns-card]').first()).toBeVisible();
            ...
            const cpexAWCard = page.locator('[data-cy=campaigns-card] big:has-text("CPEX-AW")').locator('xpath=..'); // TODO need a better way to select parent to avoid tests breaking
            ...
            const omgCard = page.locator('[data-cy=campaigns-card] big:has-text("OMG")').locator('xpath=..'); // TODO
            expect(await omgCard.locator('[data-cy=ongoing-tag]').isVisible()).toBe(false); // exist but not visible
    ...
    test.describe('platforms', () => {
        ...
        test('displays platform cards and navigates to the selected platform', async ({ page }) => {
            ...
            // check only the first element for tools, campaigns, and card
            await expect(page.locator('[data-cy=explore-tools]').first()).toBeVisible();
            await expect(page.locator('[data-cy=platforms-count]').first()).toBeVisible();
            await expect(page.locator('[data-cy=platforms-card]').first()).toBeVisible();

            const platformsCountText = await page.locator('[data-cy=platforms-count]').textContent();
            expect(platformsCountText).toMatch(/[0-9]+/i);

            const b200Card = page.locator('[data-cy=platforms-card] big:has-text("B-200")').locator('xpath=..'); // TODO
    ...
    test.describe('instruments', () => {
        ...
        test('displays instrument cards and navigates to the selected instrument', async ({ page }) => {
            ...
            await expect(page.locator('[data-cy=explore-tools]').first()).toBeVisible();
            await expect(page.locator('[data-cy=instruments-count]').first()).toBeVisible();
            await expect(page.locator('[data-cy=instruments-card]').first()).toBeVisible();
            ...
            const hamsrCard = page.locator('[data-cy=instruments-card] big:has-text("HAMSR")').locator('xpath=..'); // TODO
            ...
            const acamCard = page.locator('[data-cy=instruments-card] big:has-text("ACAM")').locator('xpath=..'); // TODO
